...
oneconvex_lenz::oneconvex_lenz(double X, double Y, double WIDTH, double HEIGHT, int KEY, double ANGLE):lenz(X, Y, WIDTH, HEIGHT, KEY, ANGLE)
{
}
oneconvex_lenz::oneconvex_lenz(oneconvex_lenz^ obj)
{
	set_key(obj->get_key());
	set_x(obj->get_x());
	set_y(obj->get_y());
...
beam ^oneconvex_lenz::reflex(beam^ obj) 
{
	double deg;
	point^ p=gcnew point();
