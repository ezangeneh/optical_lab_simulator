...
onecave_lenz::onecave_lenz(double X, double Y, double WIDTH, double HEIGHT, int KEY, double ANGLE):lenz(X, Y, WIDTH, HEIGHT, KEY, ANGLE)
{
}
onecave_lenz::onecave_lenz(onecave_lenz^ obj)
{
	set_key(obj->get_key());
	set_x(obj->get_x());
	set_y(obj->get_y());
...
beam ^onecave_lenz::reflex(beam^ obj) 
{
	double deg;
	point^ p=gcnew point();
