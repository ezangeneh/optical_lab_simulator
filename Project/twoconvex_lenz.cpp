...
twoconvex_lenz::twoconvex_lenz(double X, double Y, double WIDTH, double HEIGHT, int KEY,double ANGLE):lenz(X, Y, WIDTH, HEIGHT, KEY, ANGLE)
{
}
twoconvex_lenz::twoconvex_lenz(twoconvex_lenz^ obj)
{
	set_key(obj->get_key());
	set_x(obj->get_x());
	set_y(obj->get_y());
...
beam ^ twoconvex_lenz::reflex(beam^ obj) 
{
	double deg;
	point^ p=gcnew point();
